...
export const admin = {
  getDashboardAnalytics: (params?: { startDate?: string; endDate?: string }) => {
    ...
    return api.get<DashboardAnalytics>(`/admin/analytics${qs ? `?${qs}` : ''}`)
  },
  ...
  createUser: (body: CreateUserRequest) => api.post<{ user: AdminUser }>('/admin/users', body),

  updateUser: (id: string, body: UpdateUserRequest) =>
    api.patch<{ user: AdminUser }>(`/admin/users/${id}`, body),

  deactivateUser: (id: string) => api.patch<{ user: AdminUser }>(`/admin/users/${id}/deactivate`),

  activateUser: (id: string) => api.patch<{ user: AdminUser }>(`/admin/users/${id}/activate`),
  ...
  updateUserSubscription: (id: string, body: UpdateSubscriptionRequest) =>
    api.patch<{
      ...
      warning: string | null
    }>(`/admin/users/${id}/subscription`, body),

  listPlans: () => api.get<{ plans: AdminPlan[] }>('/admin/plans'),

  createPlan: (body: CreatePlanRequest) => api.post<{ plan: AdminPlan }>('/admin/plans', body),

  updatePlan: (id: string, body: UpdatePlanRequest) =>
    api.patch<{ plan: AdminPlan }>(`/admin/plans/${id}`, body),

  togglePlanActive: (id: string) => api.patch<{ plan: AdminPlan }>(`/admin/plans/${id}/toggle`),

  deletePlan: (id: string) => api.delete<void>(`/admin/plans/${id}`),

  listAppVersions: () => api.get<{ versions: AdminAppVersion[] }>('/admin/app-versions'),

  generateAppVersionUploadUrl: (body: GenerateUploadUrlRequest) =>
    api.post<GenerateUploadUrlResponse>('/admin/app-versions/upload-url', body),
  ...
  setCurrentAppVersion: (id: string) =>
    api.patch<{ version: AdminAppVersion }>(`/admin/app-versions/${id}/current`),

  deleteAppVersion: (id: string) => api.delete<void>(`/admin/app-versions/${id}`),
}
